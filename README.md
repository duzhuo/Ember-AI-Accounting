...
- 支持 5 种业务类型：销售收入、费用报销、资产采购、工资薪酬、借款/还款
- 自动借贷平衡校验
- 置信度评分 + 警告提示
- 凭证生命周期：草稿 → 待审批 → 过账 → 冲销
...
### 凭证管理
- 凭证列表（状态筛选：全部/草稿/待审批/已过账/已冲销）
- 凭证搜索（关键词 + 日期范围 + 金额范围）
- 凭证详情查看（含分录行）
- 草稿凭证在线编辑
- 单条过账 + 批量过账（复选框多选）
- 已过账凭证冲销（红字冲销，原子事务保证一致性）
- 导出 PDF（专业财务凭证格式）
- 附件管理：上传、查看、删除
- SAP CSV 按需导出（指定凭证或全部已过账）
...
### 角色权限控制
- 三种角色：admin（管理员）、reviewer（复核人）、user（普通用户）
- 权限矩阵：
  - admin：仅用户管理（不能操作凭证和规则）
  - reviewer：凭证审核、查看规则（不能管理用户）
  - user：生成凭证、查看规则（不能管理用户和审核凭证）
- 管理员可重置用户密码

### 移动端
- 手机友好的独立页面（mobile.html）
- 拍照/相册/拖拽上传
- 多文件批量上传 + 逐个进度
- 凭证列表筛选（全部/草稿/已过账/已冲销）
- 凭证详情查看 + 一键过账
- 完成后下载 SAP CSV
...
- 密码最少 6 位
- 文件上传大小限制 20MB
- 凭证操作归属校验（普通用户只能操作自己的凭证）
- 凭证 ID 冲突自动去重（同源凭证不同用户自动生成唯一 ID）
- 友好错误提示（不暴露内部异常）
...
- DataTable 支持操作列（自定义按钮、tooltip、点击事件）
- 凭证列表分页支持（上一页/下一页）
...
helpers/               # 工具函数
  ...
  voucher.py           # voucher_to_front、post_voucher、reverse_voucher、rules 格式化
  constants.py         # 共享常量（SUPPORTED_BUSINESS_TYPES 等）
...
routes/                # API 路由（每个文件一个 APIRouter）
  ...
  export.py            # /api/export/csv, /api/export/csv/all
  approval.py          # /api/vouchers/{id}/submit, /approve, /reject
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/export/csv?ids=id1,id2` | 按凭证号导出 SAP CSV |
